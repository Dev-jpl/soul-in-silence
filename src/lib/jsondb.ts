...
const artistFile = path.join(dataDir, 'artist.json')
...
export function initializeDatabase() {
  ...
  if (!existsSync(artistFile)) {
    const artist = {
      name: 'John Patrick Lachica',
      bio: 'Contemporary Filipino visual artist exploring vulnerability, resilience, and the emotional landscapes of human experience.',
      statement:
        'Soul in Silence is a creative practice rooted in the belief that some truths are best understood through feeling rather than explanation.',
    }
    writeFileSync(artistFile, JSON.stringify(artist, null, 2))
  }
}
...
export function getArtworks() {
  ensureDir()
  try {
    const data = readFileSync(artworksFile, 'utf-8')
...
export function getArtwork(id: number) {
  ...
  return artworks.find((a: any) => a.id === id)
}

export function updateArtwork(id: number, updates: any) {
  const artworks = getArtworks()
...
export function createArtwork(artwork: any) {
  const artworks = getArtworks()
  const newId = Math.max(...artworks.map((a: any) => a.id), 0) + 1
...
export function getArtist() {
  ensureDir()
  try {
    const data = readFileSync(artistFile, 'utf-8')
    return JSON.parse(data)
  } catch {
    return {
      name: 'John Patrick Lachica',
...
export function updateArtist(updates: any) {
  const artist = getArtist()
  const updated = { ...artist, ...updates }
  writeFileSync(artistFile, JSON.stringify(updated, null, 2))
  return updated
}
