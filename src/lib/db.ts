...
export function initializeDatabase() {
  ...
  // Initialize artist_info if empty
  const artistExists = db.prepare('SELECT COUNT(*) as count FROM artist_info').get() as {
    count: number
  }
  if (artistExists.count === 0) {
    db.prepare(`
      INSERT INTO artist_info (id, name, bio, statement)
      VALUES (1, ?, ?, ?)
    `).run(
      'John Patrick Lachica',
      'Contemporary Filipino visual artist exploring vulnerability, resilience, and the emotional landscapes of human experience.',
  ...
  const artworksCount = db.prepare('SELECT COUNT(*) as count FROM artworks').get() as {
    count: number
  }
